...
class AsyncPictify:
    ...
    async def list_video_templates(self) -> List[VideoTemplate]:
        ...
        """
        data = await self._request("GET", "/video/templates")
        return [
            VideoTemplate.model_validate(t) for t in (data or {}).get("templates") or []
        ]

    async def get_video_template_variables(self, template_id: str) -> VideoTemplateVariables:
    ...
    async def generate_video_template(
        self,
        prompt: str,
        *,
        width: int = 1080,
